...
def commissions_index():
    ...
    # Regla: la comisión sigue siendo "Nueva" hasta que el usuario visite el elemento nuevo concreto
    # (proyecto/discusión/archivo). Entrar al detalle de comisión solo marca la comisión como vista.
    is_new_by_commission_id: dict[int, bool] = {}
    commission_ids = [c.id for c in commissions]
    member_set = set(member_commission_ids)
    for cid in commission_ids:
        is_new_by_commission_id[cid] = False

    if commission_ids and member_commission_ids:
        try:
            # 1) Comisión no vista.
            seen_commission_ids = {
                row.item_id
...
def commission_detail(commission_id: int):
    ...
    try:
        ...
        # Si hay migración pendiente u otros fallos, no bloqueamos la vista.
        db.session.rollback()
    ...
    can_manage_drive_files = (
        current_user.has_permission("manage_commission_drive_files")
        or current_user.has_permission("manage_commissions")
        or user_is_privileged(current_user)
    )
    can_view_drive_history = (
        can_manage_drive_files
